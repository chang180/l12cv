            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    ...
                    <div class="mt-1 flex items-center">
                        <div x-data="{ uploading: false, progress: 0 }"
                            ...
                            <div class="flex items-center space-x-4">
                                @if ($thumbnailPreview)
                                    <img src="{{ $thumbnailPreview }}" alt="縮略圖預覽" class="h-32 w-32 object-cover rounded-md">
                                @elseif ($existingThumbnail && !$removeThumbnail)
                                    <img src="{{ $existingThumbnail }}" alt="現有縮略圖" class="h-32 w-32 object-cover rounded-md">
                                @else
                                    <div class="h-32 w-32 rounded-md bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                                ...
                                <div>
                                    <label for="thumbnail-upload" class="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-primary-600 dark:text-primary-400 hover:text-primary-500 dark:hover:text-primary-300 focus-within:outline-none">
                                        <span>上傳圖片</span>
                                                                    <input id="thumbnail-upload" wire:model="thumbnail" type="file" class="sr-only">
                                    </label>
                                    ...
                                    @if (($thumbnailPreview || $existingThumbnail) && !$removeThumbnail)
                                                                    <button type="button" wire:click="removeThumbnail" class="mt-2 text-sm text-red-600 dark:text-red-400 hover:text-red-500 dark:hover:text-red-300">
                                            移除圖片
                                        </button>
                                    @endif
                                </div>
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <flux:button wire:click="saveProject" variant="primary" class="w-full sm:w-auto sm:ml-3">
                            <flux:icon name="check-circle" class="w-5 h-5 mr-2" />
                            {{ $projectId ? '更新項目' : '創建項目' }}
                        </flux:button>
                        <flux:button wire:click="closeProjectForm" variant="ghost" class="mt-3 sm:mt-0 w-full sm:w-auto">
                            取消
                        </flux:button>
                    </div>
                </div>
